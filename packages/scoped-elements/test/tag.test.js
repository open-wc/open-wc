...
describe('tags', () => {
  describe('createUniqueTag', () => {
    it('should create a valid tag name for an anonymous class', () => {
      ...
      expect(tag).to.match(/c-\d+/);
    });

    it('should add a prefix if name is no valid', () => {
      expect(
          createUniqueTag(customElements, class Mustafar extends HTMLElement {
          })
      ).to.be.equal('c-mustafar');
      expect(
          createUniqueTag(customElements, class _Sample2 extends HTMLElement {
          })
      ).to.be.equal('c-sample2');
    });

    it('should return the name in dash case if is valid', () => {
      expect(
          createUniqueTag(customElements, class MonCalamari extends HTMLElement {
          })
      ).to.be.equal('mon-calamari');
    });
    ...
    it('should return the name in dash case and a counter if is valid and is already registered', () => {
      ...
      expect(
          createUniqueTag(customElements, class NalHutta extends HTMLElement {
          })
      ).to.match(/nal-hutta-\d+/);
    });
  });
});
