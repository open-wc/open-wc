...
This codelab will take you through web components step by step explaining each section as you go along. At the bottom of each section there is a "View final result" button, this will show you the correct code that you should en up with incase you get stuck. The steps are sequential, thus results from the previous step carry over to the next step.
...
Modify your `<body>` tag to wrap the "Hello world" message in an element called `<cool-heading>`:
...
Currently your browser does not recognize the `<cool-heading>` HTML tag. When the browser encounters an unknown HTML tag like `<cool-heading>`, it will just render it as an inline element and move on. With the custom elements API, we can tell the browser what to do with the HTML tag that we have just created.
...
To create a custom element we need declare a class that extends the `HTMLElement` class. This is the base class that powers all other native elements such as the `<input>` and `<button` elements. Now, let's go ahead and create a new class for our `<cool-heading>` element:
...
After creating our class we can associate it with a tagname by defining it in the custom elements registry. This way, whenever the browser's parser gets to the `<cool-heading>` tag, it will instantiate and apply our class to that element:
...
When the browser instantiates our custom element it triggers some lifecycle callbacks. For now, the only lifecycle methods that we are going to learn about are the following:
...
This is a native browser feature called [tagged template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#Tagged_templates). The `html` tag is just a function that gets called with information about template literal it's attached to. We won't go into the details of how it works exactly, but by using this syntax `lit-html` we can very efficiently create templates and update only the parts that change when re-rendering.
...
This shadow root is a special type of DOM node which encapsulates the elements inside of it. Styles defined inside this shadow root do not leak out, and styles defined outside the shadow root do not reach in, hence encapsulation. Also, it's not possible to use `querySelector` to select elements inside or outside the shadow root. This way we can build reusable components and be confident that they will always work the same way now matter the environment.
