...
describe('getOuterHtml', () => {
  it('support el.outerHTML also on polyfilled browsers', async () => {
    const el = await htmlFixture('<div><p>content</p></div>');
    expect(getOuterHtml(el)).to.equals('<div><p>content</p></div>');
  });

  it('supports multiple attributes', async () => {
    const el = await htmlFixture('<div id="foo" foo="bar"></div>');
  ...
  it('supports shadow dom also on polyfilled browsers', async () => {
    ...
    const el = await htmlFixture(`<${tag}><span>light content</span></${tag}>`);
    expect(getOuterHtml(el)).to.equals(`<${tag}><span>light content</span></${tag}>`);

    const el2 = await htmlFixture(`<${tag} foo="bar" foo2="bar2"><span>light content</span></${tag}>`);
    expect(getOuterHtml(el2)).to.equals(`<${tag} foo="bar" foo2="bar2"><span>light content</span></${tag}>`);
  });

  it('supports nested shadow roots', async () => {
    const innerTag = registerUniqueElement(class extends HTMLElement {
      constructor() {
        super();
        this.attachShadow({ mode: 'open' });
      }
    ...
    // no line breaks here as it's a literal comparision e.g. line breaks are considered
    const el = await htmlFixture(`<${tag}><${innerTag}><span>inner light content</span></${innerTag}><span> light content</span></${tag}>`);
    expect(getOuterHtml(el)).to.equals(`<${tag}><${innerTag}><span>inner light content</span></${innerTag}><span> light content</span></${tag}>`);

    const el2 = await htmlFixture(`<${tag} foo="bar" foo2="bar2"><${innerTag}><span>inner light content</span></${innerTag}><span> light content</span></${tag}>`);
