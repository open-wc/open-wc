...
function createTestLoaderBrowserScript(compatibilityMode, polyfills, importMap) {
  ...
  return `
(function () {
${createImportMaps(compatibilityMode, importMap)}
...
function createImportMaps(compatibilityMode, importMap) {
  const types = [];
...
function loadElement(tagName, props) {
  return new Promise(function(resolve) {
    document.head.appendChild(
      Object.assign(document.createElement(tagName), { onload: resolve, onerror: resolve }, props)
    );
  });
}
...
function loadLibsAndPolyfills(compatibilityMode, polyfillsWithoutFeatureDetection) {
  ...
  var libsAndPolyfills = polyfillsWithoutFeatureDetection;
  var testModules = [];
